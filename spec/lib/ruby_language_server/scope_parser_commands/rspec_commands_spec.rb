...
describe RubyLanguageServer::ScopeParserCommands::RspecCommands do
  ...
  describe 'blocks' do
    it 'should have a few' do
      ...
      assert_equal(['', 'describe Some::Class', 'block', 'describe some thing', 'block', 'describe inner thing', 'block', 'it is happy', 'block', 'it is not sad', 'block'], @parser.root_scope.self_and_descendants.filter_map(&:name))
    end
  end
end
